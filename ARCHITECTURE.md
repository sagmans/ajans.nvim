...
- Herdr control calls use bounded execution and decoded JSON errors. Sensitive launch and prompt payloads use Herdr's owner-only local newline-delimited JSON socket instead of process arguments. Ajans starts a persistent server with a minimal environment and sends the current process environment only with each agent launch. Workspace, tab, pane, and nested-layout changes use validated transactional cleanup.
