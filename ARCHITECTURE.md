...
| CLI API | `lua/ajans/cli/init.lua` | Public API: `select`, `show`, `toggle`, `focus`, `hide`, `close`, `send`, `prompt`, `render`, and memory-only `retry` for failed pane sends or submits. |
...
| Health | `lua/ajans/health.lua` | Check Neovim, `autoread`, the selected backend/version/server, Herdr integration status, tmux process tools, and CLI executables. |
...
state layer selects/attaches a session
        │
        ▼
authorization verifies process identity and optional screen markers
        │
        ▼
...
selected tmux or Herdr backend sends text to pane
        │
        ▼
failed send or submit becomes a memory-only retry record
```

## Tool loading

For each configured tool name, Ajans looks for `aj/cli/{name}.lua` on the runtime path. The runtime default and user config are deep-merged. Tool configs define command argv, process detection, URLs, key overrides, formatting, scroll/focus behavior, and optional Herdr screen-readiness markers.
...
- Tools can declare `mux_ready` screen markers (`required`/`blocked`) that Herdr checks against the visible pane. Blocked markers take precedence. Agy waits up to 15 seconds for its stable input footer. Authorization refusal does not create a retry record because no delivery occurred.
